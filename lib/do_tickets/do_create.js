...
function do_create(subcmd, opts, args, callback){
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function validateTypeAndTemplate(context, next){
            if(context.template.template_types.indexOf(opts.type) < 0){
            ...
            if(opts.type === 'windows'){
                customers.map(function(customer){
                    ...
                    customer.instances.map(function(instance){
                        var window_start_day = instance.window.start.format('YYYY-MM-DD');
                    ...
                    Object.keys(days).sort().map(function(day){
                        console.log('    %s', day);
                        var sorted_instances = days[day].sort(function(a, b){
                            return a.window.start - b.window.start;
                        })
                        sorted_instances.map(function(instance){
                            console.log('        Instance %s\'s window is from %s to %s',
                                instance.uuid,
                                instance.window.start.format('HH:mm:ss'),
                                instance.window.end.format('HH:mm:ss')
                            );
                        })
                    });
                })
