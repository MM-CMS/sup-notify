...
function do_create(subcmd, opts, args, callback){
    ...
    if((opts.type === 'windows_vms' || opts.type === 'windows_servers') && args[0] !== '-'){
        return callback(new Error('--type=windows_* must be used in conjunction with stdin'));
    }
...
do_create.options = [
    ...
    },
    {
        names: ['type'],
        type: 'string',
        help: '"vms", "servers", "windows_vms", "windows_servers", or "customers". ' +
            'Used to tell the tool whether the ARGS are instance/server UUIDs/hostnames or ' +
            'customer email addresses, or if the "windows" feature is to be used.'
    },
    {
        names: ['send'],
        type: 'bool',
    ...
    },
    {
        names: ['level'],
        type: 'string',
        help: '"incidents", "scheduled_maintenance", or "emergency_maintenance". ' +
