...
module.exports.notify = function(customers, template, callback){
  build_ticket_list(customers, template, function(err, tickets){
...
module.exports.notify_by_window = function(customers, template, callback){
  ...
  async.eachSeries(Object.keys(customers), function(c_uuid, callback){
    generate_csv(customers[c_uuid], function(err, windows_csv){
      upload_file(windows_csv, function(err, res){
...
var generate_csv = function(customer, callback){
  var input = [["uuid", "name", "docker_id", "date", "window_utc", "window_pdt"]];
    ...
    Object.keys(customer.windows[date_string]).sort().forEach(function(w){
      customer.windows[date_string][w].vms.forEach(function(vm){
        if(vm.internal_metadata['docker:id']){
          input.push([vm.uuid, vm.alias, vm.internal_metadata['docker:id'].slice(0,12), date.format("D-MMM-YYYY"), customer.windows[date_string][w].time_utc, customer.windows[date_string][w].time_pdt]);
        } else {
          input.push([vm.uuid, vm.alias, "", date.format("D-MMM-YYYY"), customer.windows[date_string][w].time_utc, customer.windows[date_string][w].time_pdt]);
        }
      })
    })
  });
...
var upload_file = function(file, callback){
  ...
  var opts = {
    ...
    path: "/api/v2/uploads.json?filename=migrations.csv",
  };
...
var ensure_customer_exists = function(requester, client, callback){
  client.get("/api/v2/users/search.json?query='" + encodeURIComponent(requester.email) + "'", function(err, req, res, obj){
    if(err){
      log.error(err)
      callback(err)
    } else {
      if(obj.count > 0){
        log.debug("user " + requester.email + " already exists in ZenDesk")
        callback();
      } else {
        log.debug("user " + requester.email + " doesn't exist in ZenDesk")
        client.post("/api/v2/users.json", {user: requester}, function(err, req, res, obj){
          log.debug("user " + requester.email + " created in ZenDesk")
          callback(null, obj);
        });
      }
    }
...
var build_ticket_list = function(customers, template, callback){
  var tickets = [];
...
var send_tickets = function(tickets, client, callback){
  ...
  async.eachSeries(tickets, function(ticket, callback){
    ensure_customer_exists(ticket.requester, client, function(err, created){
      if(err){
        log.error(err)
        callback(err)
      } else {
...
var ticket_body = function(customer, template){
  ...
  if(template.windows){
    ...
    Object.keys(customer.windows).sort().forEach(function(date_string){
      var date = moment(new Date(date_string.slice(0, 4), date_string.slice(4, 6)-1, date_string.slice(6, 8)));
      Object.keys(customer.windows[date_string]).sort().forEach(function(w){
        message += "##### " + date.format("D-MMM-YYYY") + ": Window " + customer.windows[date_string][w].time_utc + " UTC / " + customer.windows[date_string][w].time_pdt + " PDT\n\n";
        customer.windows[date_string][w].vms.forEach(function(vm){
