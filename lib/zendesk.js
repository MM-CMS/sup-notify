...
var customer_organisation = function(customer, client, callback){
  if(customer.zd.organization_id){
    log.info(customer.email, "part of organization; getting details")
    client.get("/api/v2/organizations/" + customer.zd.organization_id + ".json", function(err, req, res, obj){
      if(err){
        log.error(err);
        callback(err);
      } else {
...
var organisation_user_list = function(customer, client, callback){
  if(customer.zd.organization){
    ...
    if(customer.zd.organization.shared_tickets){
      log.info(customer.email, customer.zd.organization.name, 'organization tickets shared; getting users')
      var finished = false;
      var users = [];
      var pagination = {
        page: 1,
        per_page: 50,
        next_page: "/api/v2/organizations/" + customer.zd.organization.id + "/users.json"
      };
      async.until(function(){
        return finished;
      }, function(callback){
        client.get("/api/v2/organizations/" + customer.zd.organization.id + "/users.json?per_page=" + pagination.per_page + "&page=" + pagination.page, function(err, req, res, obj){
          if(err){
            log.error(err);
            callback(err);
          } else {
            if(obj.users.length > 0){
              customer.zd.organization.users = customer.zd.organization.users.concat(obj.users);
            }
            if(obj.next_page){
              pagination.page++;
            } else {
      ...
      log.info(customer.email, '(' + customer.zd.organization.name + ')', 'organization tickets not shared; skipping')
      callback();
    }
  } else {
    callback();
...
var get_user_organization_users = function(customer, client, callback){
  async.series([
    function(callback){
      customer_organisation(customer, client, function(err){
        callback(err)
      });
    },
    function(callback){
      ...
      if(err){
        ...
        if(template.organization){
          get_user_organization_users(customer, client, function(err){
            if(customer.zd.organization){
              customer.zd.organization.users.forEach(function(u){
