...
var DISABLED_REGEX = /disabled|fraud|nonpayment|abuse/;

function userIsDisabled(user) {
    return user.approved_for_provisioning === 'false' &&
        DISABLED_REGEX.test(user.email) ||
    ...
    function getCustomer(customer_email, gcnext){
        ...
        context.sdc.ufds.getUserByEmail(customer_email, function(err, user){
            ...
            if (userIsDisabled(user)) {
               log.info('ignoring disabled user or invalid email: %s', customer_email);
               return gcnext();
            }
...
function createUniqueCustomerListFromInstanceUUIDs(context, next) {
    ...
    function getCustomer(customer_uuid, gcnext){
        ...
        context.sdc.ufds.getUserEx(options, function(err, user){
            ...
            if (userIsDisabled(user)) {
               log.info('ignoring disabled user or invalid email: %s', user.email);
               return gcnext();
            }
            context.customers.push(user);
            gcnext();
            ...
            function getRoles(customer, grnext){
                ...
                function getMember(member, gmnext){
                    ...
                    context.sdc.ufds.search(member, {}, function(err, user){
                        ...
                        /*
                         * Subusers that are subscribed to notifications can have an invalid email
                         * address which causes the ZenDesk API to return a generic error.
                         *
                         * Subusers won't be disabled on their own so we only check email.
                         *
                         * ufds.search returns an array so handling it as such even though
                         * we only expect a single result.
                         */
                        var ea, invalid;
                        while (ea = user.pop()) {
                            if (!validator.isEmail(ea.email)) {
                                log.info('ignoring subuser with invalid email: %s', ea.email);
                                invalid = true;
                            }
                        }
                        if (invalid) {
