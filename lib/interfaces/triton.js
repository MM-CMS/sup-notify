...
function createUniqueWindowListFromVmUuids(context, next){
    ...
    vasync.forEachParallel({
        ...
        func: function getInstance(window, ginext){
            ...
            if(!moment.utc(window.start, DATE_INPUT_FORMAT, true).isValid()){
                return next(new Error(format('%s is not a valid start Date (for instance: %s). Input format is %s', window.start, window.instance_uuid, DATE_INPUT_FORMAT)));
            } else {
                window_start = moment.utc(window.start, DATE_INPUT_FORMAT, true);
            }

            if(!moment.utc(window.end, DATE_INPUT_FORMAT, true).isValid()){
                return next(new Error(format('%s is not a valid end Date (for instance: %s). Input format is %s', window.start, window.instance_uuid, DATE_INPUT_FORMAT)));
            } else {
                window_end = moment.utc(window.end, DATE_INPUT_FORMAT, true);
            }
