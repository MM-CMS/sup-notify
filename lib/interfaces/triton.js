...
function createUniqueInstanceListFromServerUuids(context, next) {
    ...
    var instances = [];
    log.debug('getting server details from UUIDs');
    vasync.forEachParallel({
        inputs: server_uuids,
...
function createUniqueInstanceListFromServerHostnames(context, next){
    ...
    vasync.forEachParallel({
        ...
        context.input.data = [].concat.apply([], results.successes).map(function(s){
            return s.uuid;
        });
...
function createUniqueCustomerListFromEmailAddresses(context, next){
    ...
    vasync.forEachParallel({
        inputs: customer_emails,
        func: function getCustomer(customer_email, gcnext){
            log.debug({customer_email: customer_email}, 'getting customer');
            context.sdc.ufds.getUserByEmail(customer_email, gcnext);
        }
    }, function(err, results){
        if(err){
            log.error(err);
...
function createUniqueCustomerListFromInstanceUUIDs(context, next) {
    ...
    var queue = vasync.queue(getCustomer, 5);
