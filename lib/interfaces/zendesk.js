...
function sendTickets(context, callback){
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function ticketGeneration(context, next){
            var log = context.log;
            log.info('Gathering customer information from ZenDesk')
        ...
        function ticketCreation(context, next){
            ...
            vasync.forEachPipeline({
                ...
                func: function sendTicketPipline(customer, next){
                    ...
                    vasync.pipeline({arg: ctx, funcs: [
                        ...
                        function sendTicket(context, next){
                            ...
                            /*zd_client.post('/api/v2/tickets.json', {'ticket': ticket}, function(err, req, res, obj){
                                if(err){
                                    /*
                ...
                log.info('%d tickets created', results.successes.length);
                next();
            });
        }
    ]}, callback);
...
function generateTicket(context, next){
    ...
    var notification_level = context.notification_levels[context.template.ticket_type];
    ...
    var ticket = {
        ...
        collaborators: customer.cc.map(function(c){
            return c.email
        }),
...
function buildTicketBody(customer, template){
    var body = '';
    if(template.types == 'windows_vms' || template.type == 'windows_server'){
        body += hogan.compile(template.message).render() + '\n\n';
        var days = [];
        customer.instances.map(function(instance){
            instance.tbd = (instance.window.start == 'TBD') ? true : false;
        ...
        Object.keys(days).sort().map(function(day){
            ...
            days[day].sort(function(a, b){
                ...
                body += format('        Instance %s\'s window is from %s to %s',
                    instance.uuid,
        ...
        // instances
        //body += hogan.compile(context.template.message).render(context.opts);
    }
    return body;
}
module.exports = {
