                    ...
                    function sendTicket(context, next){
                        ...
                        zendesk.tickets.create({ticket: customer.ticket}, function(err, req, ticket){
                            if(err){
                                ...
                                tickets_failed++;
                                /*
                                 * Let's sniff out whether or not this is a
                                 * ZenDesk error we can nicely format.
                                 */
                                if(err.statusCode == 422 && err.result){
...
function buildInstanceMarkdown(instance){
    ...
    function formatInstance(instance){
        ...
        var primaryNic = instance.nics.filter(function(nic){
            return nic.primary;
        })[0];
        ...
        return format('%s - %s - %s (%s) %s',
            ...
            instance.uuid,
            instance.alias,
            (primaryNic) ? primaryNic.ip : instance.nics[0].ip,
